...
- Extract the CORE role from the job posting's title, never the literal full string.
...
- Result should read like a natural, generic professional title a real person would put on their own resume — never a near-verbatim copy of the posting's title.
...
- The FIRST item in professionalExperiences (index 0, the most recent role) should use "Senior " + the same core title as index 1 (e.g. if index 1 becomes "Backend Engineer", index 0 becomes "Senior Backend Engineer") — reflecting a natural promotion from that earlier role into the current one.
...
`;
  ...
  const mergedResume = {
    ...
    title: tailoredResume.title ?? resumeData.title,
